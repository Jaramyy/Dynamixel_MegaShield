...
#define waitCom()        (Serial1.flush())          // Waits for the transmission of outgoing serial data to complete.
...
int DynamixelClass::speed(unsigned char ID, int Speed)
{
    char Speed_H,Speed_L;
    Speed_H = Speed >> 8;
    Speed_L = Speed;                      // 16 bits - 2 x 8 bits variables
	Checksum = (~(ID + AX_GOAL_LENGTH + AX_WRITE_DATA + AX_GOAL_SPEED_L + Speed_L + Speed_H))&0xFF;
...
int DynamixelClass::setTorqueLimit(unsigned char ID, int TorqueLimit)
{
    ...
    sendData(TorqueLimit_L);
    sendData(TorqueLimit_H);
    sendData(Checksum);
	waitCom();
	delayus(TX_DELAY_TIME);
	switchCom(Direction_Pin,Rx_MODE);
...
{
	Checksum = (~(ID + AX_ACCEL_LENGTH + AX_WRITE_DATA + AX_GOAL_ACCEL + Accel))&0xFF;
	switchCom(Direction_Pin,Tx_MODE);
    sendData(AX_START);                 // Send Instructions over Serial
    sendData(AX_START);
    sendData(ID);
